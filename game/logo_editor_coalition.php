...
if (isset($_POST["logo_data_update"])) {
	...
	if (strlen($logo_data)==256) {
		...
		$GAME["empire"]->coalition->data["logo"] = $logo_data;
		$GAME["empire"]->save();
	}
		
	$GAME["system"]->redirect("logo_editor_coalition.php");
}

if (isset($_POST["logodata_0_0"]))
{
	$GAME["empire"]->coalition->data["logo"] = "";
	...
	//						print "FOUND $color $i<br/>";
		...
		if ($pos == 19) $pos = "J";
		if ($pos == 20) $pos = "K";
		$GAME["empire"]->coalition->data["logo"] .= $pos;
	}
	
	
	//die($GAME["empire"]->coalition->data["logo"]);
	$GAME["empire"]->coalition->save();
	$GAME["system"]->redirect("logo_editor_coalition.php",array("NOTICE"=>T_("New logo saved")));
}

		
$GAME["template"]->setPage("logo_editor_coalition.html");

if (!$GAME["empire"]->coalition->isOwner())
	$GAME["system"]->redirect("logo_editor_coalition.php",array("WARNING"=>T_("You need to be the coalition owner to do that!")));
	
$logo_data = $GAME["empire"]->coalition->data["logo"];
...
if (isset($_GET["fill"]))
{
	...
	for ($i=0;$i<count($TPL_LOGO_COLORS);$i++)
	{
		if ($_GET["fill"] == $TPL_LOGO_COLORS[$i][0])
		{
			$base_color = $i;
			break;
		}
